...
function copyToClipboard(number) {
    navigator.clipboard.writeText(number.toString()).then(() => {
        alert(`Copied: ${number}`);
    }).catch(err => {
...
document.getElementById('subscribeForm').addEventListener('submit', function(event) {
    ...
    alert('Your subscription is confirmed. Thank you for joining us for a test drive!');
...
document.getElementById('contactForm').addEventListener('submit', function(event) {
    ...
    if (!validatePhoneNumber(number.value)) {
        showError(number, 'Number must be 10 digits');
        valid = false;
    }
